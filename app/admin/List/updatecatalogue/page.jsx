...
export default function ModifierCatalogue() {
   // const { id } = useParams();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setDialog({ open: true, message: "Catalogue modifié avec succès 🎉", type: "success" });
        } catch (error) {
            console.error(error);
            setDialog({ open: true, message: "Erreur lors de la modification !", type: "error" });
        }
    };
    ...
    const getInfos = async (id) => {
        try {
            setLoading(true);
            const response = await axios.post(`${apiUrl}/getInfosSurCatalogue`, { id });
            ...
            setSelectedFile(response.data.data[0].Photo);
        } catch (error) {
            console.error(error);
            setCatalogue(null);
        } finally {
